...
import { normalizeItemName, normalizeUnit } from "../lib/normalize";
...
function createItem(input: AddInventoryItemInput): InventoryItem {
  ...
  return {
    ...
    purchases: purchasedAt ? [{ date: purchasedAt, price: input.price ?? null, quantity: input.quantity }] : [],
    addedAt: now,
    updatedAt: now,
  };
}
...
export const resolvers = {
  Query: {
    inventory: async (_: unknown, args: { location?: InventoryItem["location"] }): Promise<InventoryItem[]> => {
  ...
  Mutation: {
    ...
    recordPurchase: async (
      ...
      const existing = all.find((i) => i.location === args.input.location && normalizeItemName(i.name) === needle);
      ...
      const updated: InventoryItem = {
        ...
        purchases: purchasedAt
          ? [...existing.purchases, { date: purchasedAt, price: args.input.price ?? null, quantity: args.input.quantity }]
          : existing.purchases,
    ...
    addToShoppingList: async (_: unknown, args: { name: string }, context: Context): Promise<ShoppingListEntry> => {
      await assertNotRateLimited(context.sourceIp);
