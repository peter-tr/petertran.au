...
import type { InventoryItem, ShoppingListEntry, PantrySettings } from "./resolvers";
...
function seed(item: Omit<InventoryItem, "id" | "addedAt" | "updatedAt" | "isStaple" | "purchases">): void {
  ...
  const purchases = item.purchasedAt ? [{ date: item.purchasedAt, price: item.price, quantity: item.quantity }] : [];
...
function createItem(input: AddInput): InventoryItem {
  ...
  return {
    ...
    purchases: input.purchasedAt ? [{ date: input.purchasedAt, price: input.price, quantity: input.quantity }] : [],
  };
}
...
export const devResolvers = {
  ...
  Mutation: {
    ...
    updateSettings: (_: unknown, args: { input: Partial<PantrySettings> }) => {
      settings = { ...settings, ...Object.fromEntries(Object.entries(args.input).filter(([, v]) => v !== undefined)) };
      return settings;
    },
  },
};
