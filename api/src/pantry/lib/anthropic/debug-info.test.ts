...
describe("buildDebugInfo", () => {
  it("computes cost from input/output token pricing", () => {
    const info = buildDebugInfo({ input_tokens: 1_000_000, output_tokens: 0 }, 123, "HAIKU");
  ...
  it("weighs output tokens 5x input tokens", () => {
    const info = buildDebugInfo({ input_tokens: 0, output_tokens: 1_000_000 }, 0, "HAIKU");

    expect(info.costUsd).toBeCloseTo(5, 10);
  });

  it("sums input and output cost", () => {
    const info = buildDebugInfo({ input_tokens: 2_000_000, output_tokens: 1_000_000 }, 0, "HAIKU");

    // 2 * $1/MTok + 1 * $5/MTok = $7
    expect(info.costUsd).toBeCloseTo(7, 10);
  });

  it("prices the SONNET tier at its own (higher) rate", () => {
    const info = buildDebugInfo({ input_tokens: 1_000_000, output_tokens: 1_000_000 }, 0, "SONNET");

    // $3/MTok + $15/MTok = $18
    expect(info.costUsd).toBeCloseTo(18, 10);
  });

  it("defaults searchesUsed/fetchesUsed to 0 when server_tool_use is absent", () => {
    const info = buildDebugInfo({ input_tokens: 10, output_tokens: 10 }, 0, "HAIKU");
  ...
  it("defaults searchesUsed/fetchesUsed to 0 when server_tool_use is null", () => {
    const info = buildDebugInfo({ input_tokens: 10, output_tokens: 10, server_tool_use: null }, 0, "HAIKU");
