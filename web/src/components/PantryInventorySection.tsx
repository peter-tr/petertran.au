...
import type { InventoryItem, PantrySettings, PantrySettingsInput, StorageLocation } from "../lib/pantryGraphql";
...
function groupItems(items: InventoryItem[], view: ViewMode): Group[] {
  ...
  if (view === "expiry") {
    ...
    const byExpiry = (a: InventoryItem, b: InventoryItem) => (a.expiresAt ?? "").localeCompare(b.expiresAt ?? "");
    soon.sort(byExpiry);
    later.sort(byExpiry);
    return [
